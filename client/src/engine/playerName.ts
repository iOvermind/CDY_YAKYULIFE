 ...
 *
 * 兩道關卡：
 * - **輸入**：寬度最多 16（八個中文字、十六個英文字母）。
 * - **顯示**：超過 16 的截成寬度 12（六個中文字）再接「……」。新名字過不了輸入那
 *   一關，會被截的只有更早的規則（最多十個字）取的名字——天梯資料庫裡還存著它們，
 *   伺服器照存不擋，所以這一關守在顯示端。
 *
 * 名字只拿來顯示，不進任何亂數或規則，所以改這兩個數字不影響重播驗證。
 */

export const NAME_MAX_WIDTH = 16;
const NAME_SHORT_WIDTH = 12;
const ELLIPSIS = '……';
