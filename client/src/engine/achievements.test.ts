...
const league = (over: Partial<LeagueCareer> = {}): LeagueCareer =>
  ({
    ...
    war: { batting: 0, pitching: 0, fielding: 0 },
    sharePoints: 0,
    awardPoints: 0,
    milestonePoints: 0,
    score: 0,
...
/** 第二人生：走上哪一條路就是哪一格，每格 3 AP（issue #39）。 */
describe('第二人生', () => {
  it('走上的那條路是一項成就', () => {
  ...
  it('同一條路走第二次不再給點', () => {
    const got = evaluateAchievements(
      ctx({ secondLife: '打火英雄', unlocked: new Set(['second_life:打火英雄']) }),
    );
    expect(got.newly.some((a) => a.id === 'second_life:打火英雄')).toBe(false);
  });
